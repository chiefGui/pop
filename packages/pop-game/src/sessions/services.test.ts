  ...
  const policy = Layer.succeed(NpcPolicy, {
    decide: () =>
      Effect.succeed(
        ...
        ].map((action) => ({ actorId: action.actorId, action })),
      ),
  });
  await Effect.runPromise(
    Effect.gen(function* () {
...
test("NPC decisions cannot impersonate another actor or control the player", async () => {
  for (const actorId of ["character:0", "character:2"] as const) {
    const policy = Layer.succeed(NpcPolicy, {
      decide: () =>
        Effect.succeed([
          {
            actorId: "character:1" as const,
            action: {
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        const simulation = yield* Simulation;
        const before = yield* simulation.getView;
        expect(yield* Effect.result(simulation.dispatch({ type: "advance-day" }))).toMatchObject({
          _tag: "Failure",
          failure: { _tag: "InvalidNpcDecision" },
        });
        expect(yield* simulation.getView).toEqual(before);
      }).pipe(Effect.provide(simulationLayer(fixture(), options, policy))),
    );
...
test("conflicting NPC creations reject the whole batch without consuming the day", async () => {
  const policy = Layer.succeed(NpcPolicy, {
    decide: () =>
      Effect.succeed(
        (["character:1", "character:2"] as const).map((actorId) => ({
          actorId,
          action: {
            type: "create-project" as const,
            actorId,
